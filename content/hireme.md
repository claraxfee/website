---
title: "Hire Me"
layout: "single"
url: "/hireme/"
summary: "Hire Clara Fee"
---

<div class="hire-hero">

## Clara Fee
**i translate between people and technology**

<div class="hire-cta">

[Contact Me](mailto:claraxfee@gmail.com)      |      [View Resume](/files/Clara_Fee_Resume.pdf)

</div>
</div>

---

## i am looking for:

- **customer-facing technical roles**: technical support engineering, solutions engineering, implementation, customer success
- based in **Philadelphia**, open to **NYC** or **fully remote**
- civic tech, edtech, and mission-driven orgs are a big plus
- **available immediately!**

---

## what's not on my resume:

**i love hard tickets!** 

Three and a half years at a college help desk means i've been called in for projector issues ten minutes before lecture starts and troubleshooted dead laptops in the middle of finals.  I stay calm, ask the right questions, and never make anyone feel dumb for lack of knowledge

**i kept a college radio station running with all-volunteer staff.** 

WHRC is entirely student run. This means our DJs usually come in with zero broadcast experience and high turnover. The only way this works is documentation and training that people actually use! I wrote it, tested it on our newbies, and rewrote it when it stopped working. This is exactly how good support teams run on their knowledge base.

**i've done requirements-gathering the hard way.** 

On the [Ticha Project](https://ticha.haverford.edu/), my 'users' were historians, linguists, librarians, and Zapotec community members. In other words, people with strong opinions about the work and no interest in WordPress internals or using GitHub. My job was to hear what they needed, translate it into something workable, and check back that I'd gotten it right. I learned to manage diverse stakeholders on a project where getting it wrong would have meant failing a community, not just a sprint.

**i write, a lot** 

My writing is public: [substack](https://clarafee.substack.com/)

**and in spanish, too.** 

I'm a Spanish literatures major, which means that beyond "conversational" I can troubleshoot, explain, and de-escalate in two languages.

---

## some examples of my work:

<div class="work-item">

### [WebApps & Technical Projects](/portfolio)
See [portfolio](/portfolio/) for details. Projects include a full-stack college calendar site, a cute Tamagotchi chrome extension to maximize productivity, and more.

</div>

<div class="work-item">

### [Ticha Project Web Design](https://ticha.haverford.edu/)
Digital humanities platform preserving Indigenous Zapotec texts. Built search interfaces and contributed to linguistic annotation tools for a digital archive of colonial-era texts. Sponsored research by Haverford College Digital Scholarship under Anna Lacy. Summer 2025.

</div>

<div class="work-item">

### WHRC Radio Infrastructure
Rebuilt the digital infrastructure for Bryn Mawr's community radio station: training documentation, scheduling systems, and broadcast tools. See this [portfolio](/portfolio/first-project/) post for more.

</div>

---

## my resume at a glance

- **IT Help Desk Student Manager (Tech Lead)**, Bryn Mawr College — 3.5 years, 300+ tickets/semester
- **CS Teaching Assistant**, Bryn Mawr & UPenn — intro programming through data structures
- **Digital Humanities & Web Design Intern**, the Ticha Project @ Haverford
- **Research**: automated unit test generation (Bryn Mawr, under Elizabeth Dinella); AI use in higher ed (under Deepak Kumar); ML pipelines (NSF REU @ Brown)
- **Station Manager**, WHRC Radio; **Budget Manager**, Batten House Sustainability Co-Op
- **B.A. Computer Science & Spanish Literature**, Film Studies minor — Bryn Mawr College, Class of 2026, 3.82 GPA

full details in the [resume PDF](/files/Clara_Fee_Resume.pdf).

---

## get in touch!

**Email:** [claraxfee@gmail.com](mailto:claraxfee@gmail.com)
**LinkedIn:** [linkedin.com/in/claraxfee](https://www.linkedin.com/in/claraxfee/)
**GitHub:** [github.com/claraxfee](https://github.com/claraxfee/)
**Substack:** [clarafee.substack.com](https://clarafee.substack.com/)